...
const EmployeeDashboard = () => {
  ...
  const [rejectModalAsset, setRejectModalAsset] = useState(null);
  const [rejectReason, setRejectReason] = useState('');
  const [actionLoadingId, setActionLoadingId] = useState(null);

  // Helper for formatting assigned and purchase dates reliably
  const getAssignedDisplayDate = (asset) => {
    const rawDate = asset?.assignedDate || asset?.assigned_date || asset?.assignedAt || asset?.assigned_at;
    if (rawDate && rawDate !== 'N/A' && rawDate !== 'null' && rawDate !== 'undefined') {
    ...
    if (asset?.purchaseDate || asset?.purchase_date) {
      const pDate = asset.purchaseDate || asset.purchase_date;
      if (pDate && pDate !== 'N/A') return pDate;
    }
    return 'N/A';
  };

  // Helper for formatting request submitted dates reliably
  const getRequestDisplayDate = (rep) => {
    const rawDate = rep?.requestDate || rep?.request_date || rep?.createdAt || rep?.created_at;
    if (!rawDate || rawDate === 'N/A' || rawDate === 'null' || rawDate === 'undefined') return '-';
    if (typeof rawDate === 'string') {
      const trimmed = rawDate.trim();
      const parts = trimmed.split(/\s+/);
      if (parts.length >= 3 && /^\d{1,2}$/.test(parts[0]) && /^[A-Za-z]{3,}$/.test(parts[1]) && /^\d{4}/.test(parts[2])) {
        return `${parts[0]} ${parts[1]} ${parts[2].replace(/\D/g, '')}`;
      }
  ...
  const [ticketCategory, setTicketCategory] = useState('hardware'); // 'hardware' | 'software'
  const [workingMode, setWorkingMode] = useState('Onsite');
  ...
  const [requestCategory, setRequestCategory] = useState('hardware'); // 'hardware' | 'software'
  const [requestWorkMode, setRequestWorkMode] = useState('Onsite');
  ...
  const [newSwReason, setNewSwReason] = useState('');
  const [newAssetPriority, setNewAssetPriority] = useState('Medium');

  // If session is empty, avoid crashing
  if (!currentUser) return null;

  // Filter items matching current logged-in employee (active assigned items)
  const myAssignedAssets = (assets || []).filter(a => {
    const assignedId = a.assignedTo || a.assigned_to;
    return assignedId && String(assignedId).trim().toLowerCase() === String(currentUser.id).trim().toLowerCase() && a.status !== 'Available' && a.status !== 'Disposed';
  });
  const pendingAssets = myAssignedAssets.filter(a => (a.acceptanceStatus === 'Pending' || a.acceptance_status === 'Pending'));
  const myRepairs = [...(repairs || []), ...(softwareTickets || [])].filter(r => r.reportedBy === currentUser.id);
  ...
  const handleRaiseSubmit = async (e) => {
    e.preventDefault();
    if (ticketCategory === 'hardware' && !repairAssetId) {
    ...
    if (ticketCategory === 'hardware') {
      ...
      const newSoftwareTicket = {
        reportedBy: currentUser.id,
        issue: repairIssue.trim(),
        description: repairDesc.trim() || `Software issue: ${repairIssue}`,
        workingMode: workingMode,
        priority: repairPriority,
        assignedTo: "IT Support Team"
      };
      await addSoftwareTicket(newSoftwareTicket);
    }
  ...
  const handleRequestAssetSubmit = async (e) => {
    ...
    if (requestCategory === 'hardware') {
      ...
      const newRepair = {
        ...
        description: `Requested new asset type: ${newAssetType}. Work Mode: ${requestWorkMode}. Priority: ${newAssetPriority}. Justification: ${newAssetReason.trim()}`,
      ...
      if (!newSwName.trim()) {
        showToast("Please enter the software name.", "error");
        return;
      }
      if (!newSwReason.trim()) {
        showToast("Please enter the justification reason.", "error");
      ...
      const newSoftwareTicket = {
        reportedBy: currentUser.id,
        issue: `Software Request: ${newSwName.trim()}`,
        description: `Requested software/license: ${newSwName.trim()}. Work Mode: ${requestWorkMode}. Priority: ${newAssetPriority}. Justification: ${newSwReason.trim()}`,
        workingMode: requestWorkMode,
        priority: newAssetPriority,
        assignedTo: "IT Support Team"
      };
      await addSoftwareTicket(newSoftwareTicket);
      logActivity("Request Software", `Requested software: ${newSwName.trim()}. Justification: ${newSwReason}`);
    }
    ...
    setRequestWorkMode('Onsite');
    setActiveModal(null);
  };

  const handleAccept = async (asset) => {
    setActionLoadingId(asset.id);
    await acceptAssetAssignment(asset.id);
    setActionLoadingId(null);
  };

  const handleRejectSubmit = async (e) => {
    e.preventDefault();
    if (!rejectModalAsset) return;
    setActionLoadingId(rejectModalAsset.id);
    await rejectAssetAssignment(rejectModalAsset.id, rejectReason.trim());
    setActionLoadingId(null);
    setRejectModalAsset(null);
    setRejectReason('');
  };

  const selectedRepairDetails = myRepairs.find(r => r.id === selectedRepairId);
  ...
  return (
    <div className="space-y-8 animate-fade-in font-sans">
      ...
      {/* Pending Asset Acknowledgment Alert Banner */}
      {pendingAssets.length > 0 && (
        <div className="bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200/80 rounded-3xl p-5 shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-4 animate-fade-in">
          <div className="flex items-center gap-3.5">
            ...
            <div>
              <h4 className="text-xs font-black text-amber-950">
                Action Required: {pendingAssets.length} New Asset Assignment{pendingAssets.length > 1 ? 's' : ''} Awaiting Acknowledgment
              </h4>
              <p className="text-[11px] text-amber-800/80 font-medium mt-0.5">
                Please review and accept or reject the hardware assigned to you by IT Administration.
              </p>
            </div>
          </div>
          <button
            onClick={() => setActiveModal('all_assets')}
            className="px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white text-xs font-bold rounded-xl shadow-sm transition-all whitespace-nowrap cursor-pointer"
          >
            Review Assignments ({pendingAssets.length})
          </button>
        </div>
      )}
            ...
            <div className="divide-y divide-slate-100 max-h-[360px] overflow-y-auto pr-1">
              {myAssignedAssets.length === 0 ? (
                ...
                myAssignedAssets.slice(0, 5).map(asset => {
                  const isPending = asset.acceptanceStatus === 'Pending' || asset.acceptance_status === 'Pending';
                  return (
                    <div key={asset.id} className="py-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3 group">
                      <div className="flex items-center gap-3 min-w-0 flex-1">
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {isPending && (
                              <span className="px-2 py-0.5 rounded-full bg-amber-100 text-amber-800 text-[8px] font-extrabold uppercase border border-amber-300 animate-pulse">
                                Pending Acceptance
                              </span>
                            )}
                          </div>
                      ...
                      <div className="flex items-center justify-between sm:justify-end gap-3 shrink-0">
                        <div className="text-left sm:text-right shrink-0">
                          ...
                          <p className="text-[10px] font-bold text-slate-700 mt-0.5">{getAssignedDisplayDate(asset)}</p>
                        </div>
                        ...
                        {isPending ? (
                          <div className="flex items-center gap-1.5 shrink-0">
                            <button
                              ...
                              className="px-2.5 py-1 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-[10px] font-bold shadow-xs flex items-center gap-1 transition-all disabled:opacity-50 cursor-pointer"
                              title="Accept asset assignment"
                            >
                              <CheckCircle2 className="h-3 w-3" />
                              <span>{actionLoadingId === asset.id ? 'Accepting...' : 'Accept'}</span>
                            </button>
                            <button
                              onClick={() => { setRejectModalAsset(asset); setRejectReason(''); }}
                              disabled={actionLoadingId === asset.id}
                              className="px-2.5 py-1 bg-rose-50 hover:bg-rose-100 text-rose-600 border border-rose-200 rounded-xl text-[10px] font-bold transition-all disabled:opacity-50 cursor-pointer"
                          ...
                          <div className="w-24 flex justify-end shrink-0">
                            <span className={`px-2.5 py-1 rounded-full text-[9px] font-extrabold uppercase tracking-wide inline-block text-center min-w-[85px] shadow-2xs ${asset.status === 'Under Repair'
                              ? 'bg-rose-50 text-rose-600 border border-rose-200/60'
                              : 'bg-blue-50 text-[#1E3A8A] border border-blue-200/60'
                              }`}>
                              {asset.status === 'Assigned' ? 'Active' : (asset.status || 'Active')}
                            </span>
                          </div>
                        )}
                      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="bg-white border border-slate-200 rounded-[2rem] max-w-lg w-full p-6 shadow-2xl space-y-4 relative z-10 animate-scale-in">
            ...
            <form onSubmit={handleRaiseSubmit} className="space-y-4">
              ...
              {/* Segmented Pill Switcher for Hardware / Software */}
              <div className="bg-slate-100/80 p-1.5 rounded-full flex items-center gap-1 border border-slate-200/60">
                <button
                  type="button"
                  onClick={() => setTicketCategory('hardware')}
                  className={`flex-1 py-2 px-4 rounded-full text-xs transition-all shadow-sm cursor-pointer text-center ${ticketCategory === 'hardware'
                ...
                <button
                  type="button"
                  onClick={() => setTicketCategory('software')}
                  className={`flex-1 py-2 px-4 rounded-full text-xs transition-all shadow-sm cursor-pointer text-center ${ticketCategory === 'software'
                    ? 'bg-white text-[#1E3A8A] shadow-md font-extrabold'
                    : 'text-slate-500 hover:text-slate-700 font-bold'
                    }`}
                >
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  placeholder={ticketCategory === 'hardware' ? "e.g. Keyboard keys stuck, Flickering screen" : "e.g. VS Code crashed, License activation failed, VPN access issue"}
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider pl-1">Work Mode *</label>
                  <select
                    value={workingMode}
      ...
      {activeModal === 'request' && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="bg-white border border-slate-200 rounded-[2rem] max-w-lg w-full p-6 shadow-2xl space-y-4 relative z-10 animate-scale-in">
            ...
            {/* Hardware vs Software Tab Selection */}
            <div className="flex rounded-full bg-slate-100 p-1 border border-slate-200">
              <button
                type="button"
                onClick={() => setRequestCategory('hardware')}
                className={`flex-1 py-2 px-4 rounded-full text-xs transition-all shadow-sm cursor-pointer text-center ${requestCategory === 'hardware'
                  ? 'bg-white text-emerald-700 shadow-md font-extrabold'
                  : 'text-slate-500 hover:text-slate-700 font-bold'
                  }`}
              >
                Hardware
              ...
              <button
                type="button"
                onClick={() => setRequestCategory('software')}
                className={`flex-1 py-2 px-4 rounded-full text-xs transition-all shadow-sm cursor-pointer text-center ${requestCategory === 'software'
                  ? 'bg-white text-emerald-700 shadow-md font-extrabold'
                  : 'text-slate-500 hover:text-slate-700 font-bold'
                  }`}
              >
            ...
            <form onSubmit={handleRequestAssetSubmit} className="space-y-4">
              ...
              {requestCategory === 'hardware' ? (
                <>
                  <div className="space-y-1">
                    ...
                    <select
                      ...
                      className="w-full p-2.5 border border-slate-200 rounded-xl text-xs focus:ring-2 focus:ring-emerald-500/20 focus:outline-none font-semibold text-slate-700 bg-slate-50/50"
                    >
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1">
                      <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider pl-1">Work Mode *</label>
                      <select
                        value={requestWorkMode}
                        onChange={e => setRequestWorkMode(e.target.value)}
                        className="w-full p-2.5 border border-slate-200 rounded-xl text-xs focus:ring-2 focus:ring-emerald-500/20 focus:outline-none font-semibold text-slate-700 bg-slate-50/50"
                      >
                        <option value="Onsite">Onsite</option>
                        <option value="Remote">Remote</option>
                  ...
                  <div className="space-y-1">
                    ...
                    <textarea
                      ...
                      className="w-full p-2.5 border border-slate-200 rounded-xl text-xs focus:ring-2 focus:ring-emerald-500/20 focus:outline-none font-medium text-slate-700"
                    />
                  </div>
                </>
              ) : (
                <>
                  <div className="space-y-1">
                    <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider pl-1">Required Software / Application Name *</label>
                    <input
                      type="text"
                      required
                      value={newSwName}
                      onChange={e => setNewSwName(e.target.value)}
                      placeholder="e.g. VS Code Enterprise, Figma, Slack Premium, Adobe CC, AWS License"
                      className="w-full p-2.5 border border-slate-200 rounded-xl text-xs focus:ring-2 focus:ring-emerald-500/20 focus:outline-none font-medium text-slate-700"
                    />
                  </div>
                  ...
                  <div className="space-y-1">
                    ...
                    <textarea
                      ...
                      required
                      value={newSwReason}
                      onChange={e => setNewSwReason(e.target.value)}
                      placeholder="State the business reasoning for requesting this software/license."
                      className="w-full p-2.5 border border-slate-200 rounded-xl text-xs focus:ring-2 focus:ring-emerald-500/20 focus:outline-none font-medium text-slate-700"
                    />
                  </div>
                </>
                ...
                {sortedRepairs.length === 0 ? (
                  ...
                  sortedRepairs.map(rep => (
                    ...
                    >
                      ...
                      <p className="text-[9px] text-slate-400 font-bold mt-1">{getRequestDisplayDate(rep)}</p>
                    </div>
                  ))
                )}
              </div>
                  ...
                  <div className="space-y-4 mt-2">
                    <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 space-y-2.5">
                      <div className="flex flex-wrap gap-2 text-[10px] font-bold text-slate-400 uppercase">
                        ...
                        <span>&bull;</span>
                        <span>{selectedRepairDetails.assetId ? `Device: ${selectedRepairDetails.assetId}` : selectedRepairDetails.workingMode ? `Mode: ${selectedRepairDetails.workingMode}` : 'Type: Software'}</span>
                        <span>&bull;</span>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="relative pl-6 space-y-4 border-l border-slate-100 ml-3">
                        {selectedRepairDetails.updates && selectedRepairDetails.updates.length > 0 ? (
          ...
          <div className="bg-white border border-slate-200 rounded-[2rem] max-w-3xl w-full p-6 shadow-2xl space-y-4 relative z-10 animate-scale-in max-h-[80vh] overflow-y-auto">
                  ...
                  {myAssignedAssets.map(asset => {
                    ...
                    return (
                      <tr key={asset.id} className="hover:bg-slate-50/30 transition-all">
                        ...
                        <td className="py-3.5 px-3 text-slate-600 font-semibold">{getAssignedDisplayDate(asset)}</td>
                        <td className="py-3.5 px-3">
                          {isPending ? (
                            <span className="px-2 py-0.5 rounded-lg text-[9px] font-extrabold uppercase bg-amber-100 text-amber-800 border border-amber-300 animate-pulse">
                              Pending Acceptance
                            </span>
                          ) : (
                            <span className={`px-2.5 py-0.5 rounded-lg text-[9px] font-extrabold uppercase border ${asset.status === 'Under Repair' ? 'bg-rose-50 text-rose-600 border-rose-200/60' : 'bg-blue-50 text-[#1E3A8A] border-blue-200/60'
                              }`}>
                              {asset.status === 'Assigned' ? 'Active' : (asset.status || 'Active')}
                            </span>
                          )}
                          ...
                          {isPending ? (
                            <div className="flex items-center justify-end gap-1.5">
                              <button
                                onClick={() => handleAccept(asset)}
                                disabled={actionLoadingId === asset.id}
                                className="px-2.5 py-1 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-[10px] font-bold transition-all disabled:opacity-50 cursor-pointer shadow-xs"
                              >
                                {actionLoadingId === asset.id ? '...' : 'Accept'}
                              </button>
                              <button
                                onClick={() => { setRejectModalAsset(asset); setRejectReason(''); }}
                                disabled={actionLoadingId === asset.id}
                                className="px-2.5 py-1 bg-rose-50 hover:bg-rose-100 text-rose-600 border border-rose-200 rounded-lg text-[10px] font-bold transition-all disabled:opacity-50 cursor-pointer"
                              >
                                Reject
                              </button>
                            </div>
                            ...
                            <button
                              ...
                              className="px-2 py-1 bg-blue-50 hover:bg-blue-100 text-blue-600 rounded-lg text-[10px] font-bold transition-all cursor-pointer"
                            >
                              Report Fault
                            </button>
                          )}
            ...
            <div className="flex justify-end gap-3 pt-3 border-t border-slate-100">
              <button
                type="button"
                onClick={() => setActiveModal(null)}
                className="px-4 py-2 border border-slate-200 rounded-xl text-xs font-bold text-slate-500 hover:bg-slate-50 hover:text-slate-700 transition-all w-full sm:w-auto cursor-pointer"
              >
                Close Inventory
              </button>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="bg-white border border-slate-200 rounded-[2rem] max-w-2xl w-full p-6 shadow-2xl space-y-4 relative z-10 animate-scale-in max-h-[80vh] overflow-y-auto">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100">
              ...
              <button onClick={() => setActiveModal(null)} className="p-1 hover:bg-slate-100 rounded-lg text-slate-400 hover:text-slate-600 transition-all cursor-pointer">
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            <div className="flex justify-end gap-3 pt-3 border-t border-slate-100">
              <button
                type="button"
                onClick={() => setActiveModal(null)}
                className="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-bold shadow-md shadow-blue-500/10 transition-all cursor-pointer"
              >
                Close Announcements
              </button>
            </div>
      ...
      {/* 7. Reject Asset Confirmation Modal */}
      {rejectModalAsset && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-sm" onClick={() => { if (!actionLoadingId) setRejectModalAsset(null); }} />
          <div className="bg-white border border-slate-200 rounded-[2rem] max-w-md w-full p-6 shadow-2xl space-y-4 relative z-10 animate-scale-in">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100">
              <h3 className="text-sm font-black text-slate-800 flex items-center gap-2">
                <AlertTriangle className="h-4.5 w-4.5 text-rose-600" />
                <span>Reject Asset Assignment</span>
              </h3>
              <button
                onClick={() => { if (!actionLoadingId) setRejectModalAsset(null); }}
            ...
            <form onSubmit={handleRejectSubmit} className="space-y-4">
              <div className="p-3 bg-rose-50/80 border border-rose-100 rounded-2xl space-y-1 text-xs">
                <p className="font-bold text-rose-900">
                  {rejectModalAsset.brand} {rejectModalAsset.model} ({rejectModalAsset.type})
                </p>
                <p className="text-[10px] text-rose-700 font-medium">
                  Asset ID: {rejectModalAsset.id} &bull; Serial: {rejectModalAsset.serialNumber}
                </p>
                <p className="text-[10px] text-slate-600 pt-1">
                  Rejecting this asset will return it to the available inventory pool and immediately notify IT Admins.
                </p>
              </div>
              ...
              <div className="space-y-1">
                ...
                <textarea
                  ...
                  placeholder="Please specify why you are declining this asset (e.g., received incorrect device specifications, device damaged, duplicate assignment, wrong model)."
                  className="w-full p-2.5 border border-slate-200 rounded-xl text-xs focus:ring-2 focus:ring-rose-500/20 focus:border-rose-400 focus:outline-none font-medium text-slate-700"
                />
              </div>
              ...
              <div className="flex items-center justify-end gap-3 pt-3 border-t border-slate-100">
                <button
                  type="button"
                  onClick={() => setRejectModalAsset(null)}
                  disabled={actionLoadingId !== null}
                  className="px-4 py-2 border border-slate-200 rounded-xl text-xs font-bold text-slate-500 hover:bg-slate-50 hover:text-slate-700 transition-all cursor-pointer disabled:opacity-50"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="px-4 py-2 bg-rose-600 hover:bg-rose-700 text-white font-bold text-xs rounded-xl shadow-md shadow-rose-500/20 flex items-center gap-1.5 transition-all cursor-pointer disabled:opacity-50"
                >
                  <X className="h-4 w-4" />
                  <span>{actionLoadingId === rejectModalAsset.id ? 'Rejecting...' : 'Confirm Rejection'}</span>
                </button>
              </div>
            </form>
          </div>
          ...
          <div className="bg-white border border-slate-200 rounded-[2rem] max-w-2xl w-full p-6 shadow-2xl space-y-4 relative z-10 animate-scale-in max-h-[80vh] overflow-y-auto">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100">
              <h3 className="text-sm font-black text-slate-800 flex items-center gap-2">
                ...
                <span>All Recent Activities ({myActivities.length})</span>
              </h3>
              <button onClick={() => setActiveModal(null)} className="p-1 hover:bg-slate-100 rounded-lg text-slate-400 hover:text-slate-600 transition-all cursor-pointer">
                <X className="h-5 w-5" />
              </button>
            ...
            <div className="space-y-4 max-h-[55vh] overflow-y-auto pr-2">
              {myActivities.length === 0 ? (
                <p className="text-xs text-slate-400 font-semibold py-8 text-center">No recent activities recorded.</p>
              ) : (
                <div className="relative pl-6 space-y-4 border-l border-slate-200 ml-3 py-1">
                  {myActivities.map((act) => {
                    const isResolve = act.activity?.includes("Resolve") || act.details?.includes("resolved");
                    const isAssign = act.activity?.includes("Assign") || act.details?.includes("assigned");
                    const isLogin = act.activity?.includes("Login") || act.details?.includes("logged in");

                    return (
                      <div key={act.id} className="relative group">
                        <span className={`absolute -left-[35px] top-1 p-1.5 rounded-full shrink-0 border-2 border-white ring-4 ring-white shadow-xs ${isResolve ? 'bg-emerald-50 text-emerald-600 ring-emerald-50/50' :
                          isAssign ? 'bg-blue-50 text-blue-600 ring-blue-50/50' :
                            isLogin ? 'bg-amber-50 text-amber-600 ring-amber-50/50' :
                              'bg-purple-50 text-purple-600 ring-purple-50/50'
                          }`}>
                          {isResolve ? <CheckCircle2 className="h-3.5 w-3.5" /> :
                            isAssign ? <Briefcase className="h-3.5 w-3.5" /> :
                              isLogin ? <Wrench className="h-3.5 w-3.5" /> : <Clock className="h-3.5 w-3.5" />}
                        </span>
                        <div className="p-3.5 bg-slate-50 hover:bg-slate-100/70 rounded-2xl border border-slate-100 transition-all space-y-1">
                          <div className="flex items-center justify-between gap-2 flex-wrap">
                            <span className="text-[10px] font-extrabold px-2 py-0.5 rounded-md bg-white border border-slate-200/80 text-slate-700 shadow-2xs">
                          ...
                          <h4 className="text-xs font-bold text-slate-800 leading-relaxed pt-0.5">{act.details}</h4>
                        </div>
                      </div>
                    );
                  })}
